...
In the equation, the backward movement is retraced. From the initial $\frac{∂C}{∂z_{i}^{l+1}}$ a backward step is taken by mutliplying $\boldsymbol{W^{l+1T}}$ to get $\frac{∂C}{∂a_{j}^{l+1}}$ (*Figure 16*). Subsequently, the error is moved through the activation function by applying an elementwise multiplication by $\boldsymbol{g(z^{l+1}}$. The 
