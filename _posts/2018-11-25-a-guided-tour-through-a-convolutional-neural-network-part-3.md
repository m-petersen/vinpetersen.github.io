...
To wrap things up, after we have initialised our weights and biases randomly in our first iteration, the resulting $$C$$ will be located on a certain point of function $$C(w_{1,2}^{(2)})$$. $$\frac{∂C}{∂w_{1,2}^{(2)}}$$ will be an easily computable derivative of this function which tells us how the gradient looks at that point and consequently how to move $$w_{1,2}^{(2)}$$ it to minimise C. 
