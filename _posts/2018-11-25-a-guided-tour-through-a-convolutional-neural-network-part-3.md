...
In other words, after we have initialised our weights and biases randomly in our first iteration, the resulting $$C$$ will be somewhere localised on function $$C(w_{1,2}^{(2)})$$ and $$\frac{∂C}{∂w_{1,2}^{(2)}}$$ will be an easily computable derivative of that function which tells us how to the gradient is at that point and consequently how to move it to minimise C. 
